...
def test (log, data_viewing):
    ...
    log("BERT modeli ve MLM Başlığı başlatılıyor...")
    base_bert = BERT(
    ...
    log(f"Modeldeki toplam parametre sayısı: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
    ...
    for epoch in range(1, 21):
        ...
        log(f"Epoch: {epoch:02d} | Loss: {loss.item():.4f}")
